...
function badgeStatus(status: string): React.ReactNode {
  switch (status) {
    case "aberto":
      return <span className="badge-green">Inscrições abertas</span>;
    case "em_breve":
...
// O React recria o tipo a cada render do pai, causando desmontagem/remontagem
// desnecessária e loops de efeito.
...
function CardEvento({
  ...
  return (
    <>
      <div
        className={`bg-white rounded-2xl shadow-card border border-surface-200 px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${
      ...
      >
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          {realizado ? (
            <div
              title="Evento já realizado — edição não permitida"
          ...
          <button
            ...
            onClick={() => onAbrirConfirmacao(evento.id)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 animate-fade-in">
          <div className="bg-white rounded-2xl shadow-modal p-8 max-w-sm w-full mx-4 text-center">
            ...
            <div className="flex gap-3">
              <button onClick={onFecharConfirmacao} className="btn-secondary flex-1">
                Cancelar
              </button>
              <button
...
export default function AdminEventos() {
  ...
  // ─── Busca com AbortController — evita loading infinito ──────────────────────
  useEffect(() => {
    let mounted = true;
  ...
  // O browser throttla timers em abas inativas — ao voltar ao foco,
  // cancela a query anterior (via cleanup do useEffect) e inicia uma nova.
  useEffect(() => {
          ...
          <div className="flex flex-col gap-3">
            ...
            {eventosFuturos.map((evento) => (
              <CardEvento
                ...
                onAbrirConfirmacao={setConfirmandoId}
                onFecharConfirmacao={() => setConfirmandoId(null)}
