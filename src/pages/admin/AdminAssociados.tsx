  ...
  async function handleImportacao(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (error) {
        ...
        setReloadKey(k => k + 1)
        mostrarFeedback(
          `Importação concluída: ${data.criados} criados, ${data.atualizados} atualizados, ${data.inativados} inativados.`
        )
      }
    } catch {
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-6 pt-28 pb-12">
        ...
        {resultadoImport && (
          <div className="bg-green-50 border border-green-200 rounded-xl px-5 py-4 mb-6 text-sm text-green-800">
            ...
            <p>
              {resultadoImport.criados} criados · {resultadoImport.atualizados} atualizados ·{' '}
              {resultadoImport.inativados} inativados · {resultadoImport.erros?.length ?? 0} erros
            </p>
          </div>
        )}
