...
export default function AdminFormEvento() {
  ...
  useEffect(() => {
    ...
    const buscar = async () => {
      ...
      // Bloqueia edição de evento já realizado
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.inicio_inscricao)
      return setErro("Informe o início das inscrições.");
    if (!form.fim_inscricao) return setErro("Informe o fim das inscrições.");
    ...
    if (form.fim_inscricao >= form.data_evento)
      return setErro(
        "O fim das inscrições deve ser anterior a data do evento.",
      );
      ...
      <main className="flex-1 max-w-3xl w-full mx-auto px-4 sm:px-6 pt-28 pb-12">
        ...
        >
          <div className="flex flex-col gap-5">
            ...
            <div className="grid grid-cols-[180px_1fr] items-center gap-4">
              <label className="label mb-0 text-right">
                Inicio das inscricoes:
              </label>
              <input
                type="datetime-local"
                ...
                min={dataMinima}
                onChange={(e) =>
                  handleChange("inicio_inscricao", e.target.value)
                }
                className="field"
              />
            </div>
